...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 */
	public function change_password($identity, $old, $new, $confirm)
	{
		if ($new !== $confirm) {
			$this->set_error('Password and confirm password does not match');
			return false;
		}
		$this->trigger_events('pre_change_password');
	...
	public function user_uuid($uuid = NULL)
	{
		$this->trigger_events('user');

		$uuid = isset($uuid) ? $uuid : $this->session->userdata('uuid');

		$this->limit(1);
		$this->order_by($this->tables['users'].'.uuid', 'desc');
		$this->where($this->tables['users'].'.uuid', $uuid);
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'ion_auth') || $group->name == $this->config->item('default_group', 'ion_auth'))
		{
